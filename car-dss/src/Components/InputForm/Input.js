...
function Input() {
  ...
  const apiPayload = {
    ...
    "Engine HP": selectedHP,
    "Year": desiredPrice,
    "Highway Percent": selectedTimeHighway / 100,
    "Vehicle Size": null,
    "Engine Fuel Type": selectedFuel,
    "Transmission Type": selectedTrans,
    "Driven Wheels": selectedDrivetrain
  };

  const mapUserInputToAPIPayload = async () => {
    if (selectedStyle === "Coupe" || selectedStyle === "Convertible" || selectedStyle === "Sedan" || selectedStyle === "Wagon") {
    }

    apiPayload["MSRP"] = desiredPrice;
    apiPayload["MPG"] = selectedMileage;
    apiPayload["Number of Doors"] = selectedDoors;
    apiPayload["Engine HP"] = selectedHP;
    apiPayload["Year"] = selectedYear;
    apiPayload["Highway Percent"] = selectedTimeHighway / 100;
    apiPayload["Vehicle Size"] = selectedCarsize;
    apiPayload["Engine Fuel Type"] = selectedFuel;
    apiPayload["Transmission Type"] = selectedTrans;
    apiPayload["Driven Wheels"] = selectedDrivetrain;
